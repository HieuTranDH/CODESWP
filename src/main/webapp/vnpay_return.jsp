                        ...
                        <%
                            if (signValue.equals(vnp_SecureHash)) {
                                if ("00".equals(request.getParameter("vnp_TransactionStatus"))) {
                                    ...
                                    // Nếu không thành công, gọi hàm deleteTicket để xóa vé từ DB
                                    Integer ticketId = (Integer) session.getAttribute("ticketId");
                                    if (ticketId != null) {
                                        Ticket_DB ticketDAO = new Ticket_DB();
                                        ticketDAO.deleteTicket(ticketId);
                                    }
                                    // Xóa ticketId và totalPrice khỏi session
                                    session.removeAttribute("ticketId");
                                    session.removeAttribute("totalPrice");
                                }
                            } else {
                                ...
                                // Nếu chữ ký không hợp lệ, cũng gọi deleteTicket để xóa vé từ DB
                                Integer ticketId = (Integer) session.getAttribute("ticketId");
                                if (ticketId != null) {
                                    Ticket_DB ticketDAO = new Ticket_DB();
                                    ticketDAO.deleteTicket(ticketId);
